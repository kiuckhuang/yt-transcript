...
# sample URL:
# https://www.youtube.com/watch?v=vT1_kGcNf7A
YT_URL=$1
video_id=''


# Check if input is a valid YouTube video ID (11 chars: letters, numbers, _ or -)
if [[ "$YT_URL" =~ ^[A-Za-z0-9_-]{11}$ ]]; then
    video_id=${YT_URL}
fi
...
if [[ $YT_URL =~ $regex ]]; then
    video_id="${BASH_REMATCH[3]}"
    #echo "Video ID: $video_id"
fi

if [ -z "${video_id}" ]; then
    echo "Input is not a valid YouTube video URL or ID."
    echo "$0 YUTUBE_URL"
    exit 1;
fi
